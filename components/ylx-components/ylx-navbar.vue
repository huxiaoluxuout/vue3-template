...
<script>
	import {
		...
		getPages
	} from "@/components/ylx-components/ylx-JS/common";

	let menuButtonInfoALI = null,
		systemInfo = null,
		pages = null;
	...
	export default {
		...
		computed: {
			rightWidth() {
				return (!this.configNavBar_.hiddenLeftIcon && !this.configNavBar_.isTabBarPage) ? (this.menuButtonWidth + 15) +
					'px' : this.menuButtonWidth + 'px'
			},
			defaultLeftIconName() {
			...
			navbarHeight() {
				// 10 标题到底部之间的距离
				let navbarH = this.bottomGap + this.menuButtonTop + this.statusBarHeight + this.menuButtonHeight + this
					.liuHaiHeight
			...
			configNavBar_() {
				...
				return Object.assign({
					...
					hiddenLeftIcon: this.hiddenLeftIcon
				});
			},
			...
			leftIconClick() {
				if (this.isLeftClick) {
					...
					if (this.pageHierarchy > 1) {
						if (typeof this.leftClickFn === 'function') {
							this.leftClickFn()
						}
						uni.navigateBack({
							delta: 1
