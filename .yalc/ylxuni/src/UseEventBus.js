...
export class UseEventBus {
    ...
    constructor(platform) {
        ...
        this.onGlobal = this.handleRegisterGlobalEvent
        this.emitGlobal = this.handleSendGlobal
        this.emit = this.handleEmit
        this.on = this.handleOn
    }
     ...
     */
    handleRegisterGlobalEvent(globalCallback = UseEventBus.defaultGlobalCallback) {
     ...
     */
    async handleEmit({targetPath, options = {}, source = ''},
     ...
     */
    async handleSendGlobal(options = {}, source = '') {
        const currentRoute = await UseEventBus.getRoute();
        const mergedOptions = typeof options === 'object' ? Object.assign({fromPage: currentRoute}, options) : options;
